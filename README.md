...
>
> *Drawn to the final paper's structure — Stabilize §IV, Assess §V, Govern §VI, Enforce §VII, evaluated in §VIII — and regenerated by [`scripts/make_architecture_figure.py`](scripts/make_architecture_figure.py). The manuscript's own Fig. 1 is preserved unaltered as the version of record at [`paper/public/figures/sage_framework.pdf`](paper/public/figures/sage_framework.pdf); its inset pointers predate the final section numbering, so this diagram is used here to keep the README consistent with the article it documents.*
