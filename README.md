...
> **Fig. 1 of the paper** — the SAGE framework for deployment-oriented analysis of LLM-based autonomous agents. The four pillars connect stability monitoring, cost-aware assessment, protocol governance, and bounded corrective control through a feedback path from monitor signals to interventions. Vector source: [`paper/public/figures/sage_framework.pdf`](paper/public/figures/sage_framework.pdf).
>
> *Note: the small section pointers inside the figure (`§III`, `§IX`, `§XI`, …) follow an earlier draft's numbering. The authoritative mapping to the final paper is the [paper-to-code table](#paper-to-code-map) below.*
...
├── docs/assets/                    # Paper figures (PNG)
├── paper/public/figures/           # Paper figures (vector PDF)
